...
KDNode3::KDNode3(int d, double cv, KDNode3 *parent, KDNode3 *hi_c, KDNode3 *lo_c,
...
KDNode3* KDNode3::build_kdtree(const std::list<Triangle> *tris, 
                               unsigned int bucketSize,   // defaults to 1
                               int level,                 // defualts to 0 == root
                               KDNode3 *parent)            // defaults to NULL    
{
    ...
    Spread3* spr = KDNode3::spread(tris); // call to static method which returns new object
    ...
    if ( (tris->size() <= bucketSize) || (spr->val == 0.0)) {
        KDNode3 *bucket;
        bucket = new KDNode3(spr->d, 0.0 , parent , NULL, NULL, tris, level);
    ...
    */
    ...
    KDNode3 *node = new KDNode3(spr->d, cutvalue, parent, NULL,NULL,NULL, level);
    
    // create the child-nodes:
    // recursion:                   list    bucketsize   level   parent
    node->hi = KDNode3::build_kdtree(hilist, bucketSize, level+1, node);
    node->lo = KDNode3::build_kdtree(lolist, bucketSize, level+1, node);    
...
int KDNode3::cutcount=0;

/// find the maximum 'extent' of Triangle list tris along dimension d
Spread3* KDNode3::spread(const std::list<Triangle> *tris) {
    //Bbox minvals;
    //Bbox maxvals;
    ...
    if ( tris->empty() ) {
        ...
        for (unsigned int m=0;m<6;++m) {   // dim,  spread, start
            spreads.push_back( new Spread3(m , maxval[m]-minval[m], minval[m] ) );  
        }
...
void KDNode3::search_kdtree( std::list<Triangle>* tris,      // found triangles added to tris
                            ...
                            const unsigned int plane)        // the search plane, defaults to 0 = all.
{
    ...
    if ( (node->dim % 2) == 0) { // cutting along a min-direction: 0, 2, 4
        ...
        if ( (KDNode3::plane_valid(node->dim,plane)) && (node->cutval > bb[maxdim]) ) { // search only lo
            KDNode3::search_kdtree(tris, bb, node->lo, plane);
        } else { // need to search both child nodes
            KDNode3::search_kdtree(tris, bb, node->hi, plane);
            KDNode3::search_kdtree(tris, bb, node->lo, plane);
        }
    } else { // cutting along a max-dimension: 1,3,5
        unsigned int mindim = node->dim-1;
        if ( (KDNode3::plane_valid(node->dim,plane)) && (node->cutval < bb[mindim]) ) { // search only hi
            KDNode3::search_kdtree(tris, bb, node->hi, plane);
        } else { // need to search both child nodes
            KDNode3::search_kdtree(tris, bb, node->hi, plane);
            KDNode3::search_kdtree(tris, bb, node->lo, plane);
...
bool KDNode3::plane_valid(const unsigned int dim,const unsigned int plane) {
    // search in the KD-tree only if the plane variable says so.
    // plane=0  means search all dimensions            dims=[ 0 1 2 3 4 5 ]
    // plane=1 search in the yz plane (perp to x-dir)  dims=[ _ _ 2 3 4 5 ]
    // plane=2 search in the xz plane (perp to y-dir)  dims=[ 0 1 _ _ 4 5 ]
