...
void SmartWeave::add_all_edges() {
    ...
    BOOST_FOREACH (Vertex &vertex, vertices) {
        if ((g[vertex].type == INT) || (g[vertex].type == FULLINT)) {
            ...
            boost::tie(x_u, x_l) =
                find_neighbor_vertices(VertexPair(vertex, g[vertex].position.x), *(g[vertex].xi), false);
            boost::tie(y_u, y_l) =
            ...
            for (in_edge_itr = in_edges.begin(), out_edge_itr = out_edges.begin();
                 in_edge_itr < in_edges.end() && out_edge_itr < out_edges.end(); in_edge_itr++, out_edge_itr++) {
