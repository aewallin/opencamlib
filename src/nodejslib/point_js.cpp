...
PointJS::PointJS(const Napi::CallbackInfo& info) : Napi::ObjectWrap<PointJS>(info) {
  ...
  if (length == 2) {
    ...
    this->actualClass_ = ocl::Point(x.DoubleValue(), y.DoubleValue());
  } else if (length == 3) {
    Napi::Number x = info[0].As<Napi::Number>();    
    Napi::Number y = info[1].As<Napi::Number>();
    Napi::Number z = info[2].As<Napi::Number>();
    this->actualClass_ = ocl::Point(x.DoubleValue(), y.DoubleValue(), z.DoubleValue());
  }
}
...
void PointJS::Add(const Napi::CallbackInfo& info) {
  ...
  this->actualClass_.operator+=(ocl::Point(oclp.x, oclp.y, oclp.z));
}