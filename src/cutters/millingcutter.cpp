...
bool MillingCutter::facetDrop(CLPoint &cl,
                              const Triangle &t) const { // Drop cutter at (cl.x, cl.y) against facet of Triangle t
...
bool MillingCutter::generalFacetPush(double normal_length, double center_height, double xy_normal_length,
    ...
    if (fib.p1.y == fib.p2.y) { // XFIBER
        ...
        double tval =
            (1.0 / (fib.p2.x - fib.p1.x)) * (t.p[0].x + normal_length * normal.x + xy_normal_length * xy_normal.x -
        ...
        double tval =
            (1.0 / (fib.p2.y - fib.p1.y)) * (t.p[0].y + normal_length * normal.y + xy_normal_length * xy_normal.y -
