...
def drawVertex(myscreen, p, vertexColor):
    myscreen.addActor( camvtk.Sphere( center=(p.x,p.y,p.z), radius=1, color=vertexColor ) )

def drawEdge(myscreen, e):
    p1 = e[0]
    p2 = e[1]
    myscreen.addActor( camvtk.Line( p1=( p1.x,p1.y,p1.z), p2=(p2.x,p2.y,p2.z), color=camvtk.cyan ) )
...
if __name__ == "__main__":  
    ...
    #print vd
    p=ocl.Point(10,20)
    #vd.add_vertex(p , ocl.VoronoiVertexType.UNDECIDED )
    #print vd
    
    vd.addVertexSite( p )
    
    for v in vd.getVertexSites():
        drawVertex(myscreen, v, camvtk.green)
    
    for v in vd.getFarVertices():
        drawVertex(myscreen, v, camvtk.pink)
        
    for v in vd.getVertices():
        drawVertex(myscreen, v, camvtk.red)
        
    for e in vd.getEdges():
        drawEdge(myscreen,e)
